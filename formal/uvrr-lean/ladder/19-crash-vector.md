...
Following Michael, Ports, Sharma and Szekeres (2017), Algorithm 1 and Definition 6. Pointwise vector join and pruning derive pairwise incarnation consistency for every reachable collector state. Distinct sender identities alone admit an inconsistent quorum; a current replacement restores collector quorum size. This does not prove quorum-knowledge persistence, full recovery, protocol repair or network liveness. See evidence/crash-vector/README.md for the published antecedent and remaining obligations.

```bash
cat UVRR/CrashVector.lean
```
...
```bash
lake env lean UVRR/CrashVector.lean
```
...
#print axioms CrashVector.reachable_consistent
#print axioms CrashVector.old_reply_rejected
#print axioms CrashVector.current_reply_retained
#print axioms CrashVector.Example.unfiltered_quorum_inconsistent
#print axioms CrashVector.Example.rejects_old_then_accepts_current
LEAN
```

```output
'CrashVector.reachable_consistent' depends on axioms: [propext, Quot.sound]
'CrashVector.old_reply_rejected' depends on axioms: [propext, Quot.sound]
'CrashVector.current_reply_retained' depends on axioms: [propext, Quot.sound]
'CrashVector.Example.unfiltered_quorum_inconsistent' depends on axioms: [propext, Quot.sound]
'CrashVector.Example.rejects_old_then_accepts_current' depends on axioms: [propext, Quot.sound]
```
