# Rung 18: Unresolved repeated-recovery counterexample

*2026-09-06T08:35:56Z by Showboat 0.6.1*
<!-- showboat-id: 0828ebfb-9b24-4d9c-aaf4-0f8da8c2f974 -->

This rung records an unresolved failure, not a safety proof. Unchanged Rust production code commits different operations at slot 3 after serial amnesiac recoveries, authentic delayed first-round/view-change packets, ordinary timeouts and fresh recovery ticks. A separate directed TLC witness is archived alongside it. The runner succeeds only when the archived safety assertion fails with the exact committed-divergence diagnostic. No production fix has been made; component theorems must not be presented as an end-to-end proof.

```bash
cat evidence/delayed-fence/reproduction.rs
```
...
```bash
python3 check_recovery_counterexample.py
```
