...
theorem useBeforeDurable_not_unique (flushed : Nat → Bool) (k : Nat)
  ...
  intro huniq
  have h := huniq 1 k (k + 1) (useBeforeDurable_recycles flushed 1 k hk)
  rcases h with ⟨hsys, hk'⟩
  omega
...
theorem useBeforeDurable_unique_when_flushed :
  ...
  have hsys : s₁ = s₂ := by
    have := congrArg Identity.systemId heq
    simpa [useBeforeDurable] using this
  have hcnt : durableCount (fun _ => true) k₁ + 1 = durableCount (fun _ => true) k₂ + 1 := by
    have := congrArg Identity.counter heq
    simpa [useBeforeDurable] using this
  have hcnt' : durableCount (fun _ => true) k₁ = durableCount (fun _ => true) k₂ := by omega
  have hmono : ∀ a b, a < b → durableCount (fun _ => true) a < durableCount (fun _ => true) b := by
    intro a b hlt
    induction' hlt with b hlt ih
    · unfold durableCount; simp
    · unfold durableCount; simp [ih]
  by_cases h : k₁ < k₂
  · have := hmono k₁ k₂ h
    omega
  · by_cases h' : k₂ < k₁
...
theorem wire_requires_bump
  ...
  cases h
  · -- .dirty → .reincarnating is impossible; no such constructor
    nomatch h
  · -- .bumped → .reincarnating: we are in bumped
    assumption
  · -- .reincarnating: must have come from .bumped
    assumption
