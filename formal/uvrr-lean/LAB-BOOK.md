...
Tighter cost control and persistent checkpoints were then required.
...
artifact subtree, without committing unrelated staged changes.
...
staged `tools/tla2tools` submodule directory and matches an indexed
...
The preceding work was generated by GLM-5.3-Flash followed by Claude
Fable 5.1, with rollout artifacts under `.tmp/rollouts/` and the report
`formal/lean-leanstral-report.md`. This is recorded provenance, not a new audit
...
failure remains documented above. No Rust implementation or staged
...
repeated. Unrelated staged changes remain outside this work.
...
are recorded in provenance.json, including the preserved staged Cargo.toml state.
...
The manuscript was then converted to an independently editable format credited
to Simon Massey (simon.massey@stenographer.cloud), mirroring David Turner's
paper layout. Compared the downloaded primary PDF's first two
...
No affiliation or license was invented.
...
actual crash-vector acquisition transitions. Preserve forthcoming
...
## 2026-09-07 P — reincarnation documentation

The Crash-Stop-Self-Evict
...
prominent, and the identity pair (old, new) is stated to supersede
rung 17's `generation` ghost as freshness carrier where they meet, with the
RecoveryFence machinery retained and rung 17 untouched. Speculative
