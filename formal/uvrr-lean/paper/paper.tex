...
\subsection{The committed batches}
Table~\ref{tab:three} is the three-node schedule: $n_0$ fails and is replaced by its fresh identity $n_3$, while $n_1,n_2$ survive. A join inserts a zero-weight identity; an increment grants its voting weight. A leave removes the old identity after its weight reaches zero.

\begin{table}[t]
\centering
\caption{Three-node Crash-Stop-Reincarnation: two committed batches. Zero denotes voting weight, including absent identities.}
\label{tab:three}
\small
\begin{tabular}{lrrrr}
\toprule
...
\subsection{The forced sequence}
