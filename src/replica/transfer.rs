...
impl<J: Journal, Q: QuorumStrategy> Replica<J, Q> {
    ...
    pub(in crate::replica) fn plan_new_state(
        ...
        let header = message.header;
        trace!("NEW_STATE in: from={:?} view=({:?},{:?}) through={:?} committed={:?} more={} entries={} | self: current=({:?},{:?}) retained=({:?},{:?}) accepted={:?} committed={:?} status={:?} table_era={:?}",
            ...
            self.progress.status(), self.progress.config().current().era);
        ...
        trace!("NEW_STATE qualified={} boot_fence={} fetch={:?} header=({:?},{:?})",
            qualified, boot_fence, self.transfer.map(|f| f.view), header.view.era, header.view.view);
        if !qualified {
            return self.drop_plan(
        ...
        let boot_acquisition = header.view == current
            ...
            && current == self.progress.retained();
        trace!("NEW_STATE current_view_transfer={} boot_acquisition={}",
        ...
        trace!("NEW_STATE take={:?} (cap={:?}) new_accepted={:?} new_committed={:?}",
        ...
        trace!("NEW_STATE candidate: accepted={:?} committed={:?} status={:?} table_era={:?}",
