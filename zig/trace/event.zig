...
const Peer = enum(u8) { client, replica }; // PATCH: TB vsr.Peer (network peers) is out of scope
const Zone = vsr.Zone;
const Duration = stdx.Duration;

const Operation = enum(u8) { pulse, };


const TreeEnum = enum(u32) { dummy, };


const GrooveEnum = enum(u32) { dummy, };
...
const EventOperationData = struct {
    ...
    pub fn from(operation: @TypeOf(Operation.pulse)) EventOperationData {
...
pub const Event = union(enum) {
    replica_commit: struct { stage: u8, op: ?usize = null },
...
pub const EventTiming = union(Event.Tag) {
    replica_commit: struct { stage: u8 },
    replica_aof_write,
    replica_aof_checkpoint,
    replica_sync_table,
    replica_request: EventOperationData,
    ...
    pub fn slot(event: *const EventTiming) u32 {
        switch (event.*) {
            ...
            inline .replica_commit => |data| {
                const stage = data.stage; // PATCH: stage is a plain u8 (no CommitStage)
...
test "EventTiming slot doesn't have collisions" {
    ...
    while (!g.done()) {
        const event: EventTiming = switch (g.enum_value(Event.Tag)) {
            .replica_commit => .{ .replica_commit = .{ .stage = g.int(u8) } },
