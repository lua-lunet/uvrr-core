...
work=root/'.tmp/uvrr-audit/leanstral-small'
env=os.environ.copy()
secrets=[]
...
argv=['/Users/consensussolutions/opt/mistral-vibe-fork/bin/vibe','-p',(work/'PROMPT-weighted-general.txt').read_text(),'--agent','lean','--auto-approve','--trust','--max-turns','6','--max-price','0.50','--max-tokens','150000','--output','streaming','--workdir',str(work)]
...
with raw.open('w') as out:
    ...
    while p.poll() is None:
        ...
        if time.monotonic()-started>120 or rss>2*1024*1024:
            reason='wall_timeout' if time.monotonic()-started>120 else 'memory_limit'
...
report={'agent':'lean','model_from_builtin_profile':'labs-leanstral-1-5','max_turns':6,'max_price_usd':0.5,'max_total_tokens':150000,'wall_limit_seconds':120,'tree_rss_limit_kib':2*1024*1024,'peak_tree_rss_kib':maxrss,'termination':reason,'returncode':rc,'elapsed_seconds':round(time.monotonic()-started,2)}
